 ...
 */
void PQCLEAN_HQC192_CLEAN_hqc_pke_keygen(uint8_t *ek_pke, uint8_t *dk_pke, uint8_t *seed) {
 ...
 */
void PQCLEAN_HQC192_CLEAN_hqc_pke_encrypt(ciphertext_pke_t *c_pke, const uint8_t *ek_pke, const uint64_t *m, const uint8_t *theta) {
   shake256_xof_ctx theta_xof_ctx = {0};
    ...
    PQCLEAN_HQC192_CLEAN_vect_mul_2(c_pke->u, tmp, r2, h, s);
    vect_add(c_pke->u, r1, c_pke->u, VEC_N_SIZE_64);

    // Compute v = C.encode(m)
    PQCLEAN_HQC192_CLEAN_code_encode(c_pke->v, m);
 ...
 * @returns 0
 */
uint8_t PQCLEAN_HQC192_CLEAN_hqc_pke_decrypt(uint64_t *m, const uint8_t *dk_pke, const ciphertext_pke_t *c_pke) {
    uint64_t y[VEC_N_SIZE_64] = {0};
    ...
    // Compute u.y
    PQCLEAN_HQC192_CLEAN_vect_mul(tmp1, y, c_pke->u);
    // Truncate(u.y)
    vect_truncate(tmp1);
