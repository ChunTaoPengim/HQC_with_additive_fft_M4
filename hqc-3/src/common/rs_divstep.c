...
{
    // reverse
    uint8_t g[2*(PARAM_DELTA+2)] = {0};
    memcpy(g, syndromes, 2*PARAM_DELTA);
    // 
    uint8_t v[2*(PARAM_DELTA+2)] = {0};
    uint8_t r[2*(PARAM_DELTA+2)] = {0};
    uint8_t f[2*(PARAM_DELTA+2)] = {0};
    r[0] = 1;
    f[(2*PARAM_DELTA)] = 1;
    int delta = 0;
    for (int i = 0; i < 2*PARAM_DELTA; i++) {
        ...
        uint8_t temp;
        for(int j = 0; j < 2*(PARAM_DELTA+2); j++)
        {
            temp = f[j] ^ g[j];
        ...
        // {
        //     g[j] = PQCLEAN_HQC192_CLEAN_gf_mul(g[j], f0) ^ PQCLEAN_HQC192_CLEAN_gf_mul(f[j], g0);
        // }
        ...
        // for(int j = 0; j < 2*(PARAM_DELTA) +1; j++)
        // {
        //     r[j] = PQCLEAN_HQC192_CLEAN_gf_mul(r[j], f0) ^ PQCLEAN_HQC192_CLEAN_gf_mul(v[j], g0);
        // }
...
void compute_error_values_new(uint8_t *error_values, const uint8_t *omega, const uint8_t *sigma, const uint8_t *error) {
    ...
    uint8_t beta_j_inverse[PARAM_DELTA] = {0};
    ...
    uint16_t mask1;
    uint16_t mask2;
    uint32_t tmpp1_array[PARAM_DELTA >> 2] = {0};
    uint32_t tmpp2_array[PARAM_DELTA >> 2] = {0};
    ...
    gf256_madd_16(tmpp2_array, beta_j_inverse_u32, sigma_deriv[(PARAM_DELTA-1)]);
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        for(int j = PARAM_DELTA-3; j >= 0; j-=2) // PARAM_DELTA = 16 in HQC-3
        {
    ...
    {
        e_j[i] = PQCLEAN_HQC192_CLEAN_gf_mul(tmpp1_array_bytes[i], PQCLEAN_HQC192_CLEAN_gf_inverse(tmpp2_array_bytes[i]));
    } 
