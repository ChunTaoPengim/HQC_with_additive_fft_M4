...
/////////////////////////////////////////
//
// Optimized version for n_bits = 16384, 32768, 65536
//
/////////////////////////////////////////
...
static void _bc_1_16384( uint32_t *poly )
{
  repr_s8_1_16384( poly );

  bc_256( poly , 16384/256 );

  bc_1_256( poly , 16384/256 );
}

static void _ibc_1_16384( uint32_t *poly )
{
  ibc_1_256( poly , 16384/256 );

  ibc_256( poly , 16384/256 );
...
static void _bc_1_32768( uint32_t *poly )
{
  repr_s8_1_32768( poly );

  bc_256( poly , 32768/256 );

  bc_1_256( poly , 32768/256 );
}

static void _ibc_1_32768( uint32_t *poly )
{
  ibc_1_256( poly , 32768/256 );

  ibc_256( poly , 32768/256 );
...
static void _bc_1_65536( uint32_t *poly )
{
  repr_s8_1_65536( poly );

  bc_256( poly , 65536/256 );

  bc_1_256( poly , 65536/256 );
}

static void _ibc_1_65536( uint32_t *poly )
{
  ibc_1_256( poly , 65536/256 );

  ibc_256( poly , 65536/256);

  irepr_s8_1_65536( poly );
}

//
// div by  s16(x) = x^{65536} + x
//
...
static void _bc_1_131072( uint32_t *poly )
{
  repr_s16_1_131072(poly);
  _bc_1_65536(poly);
  _bc_1_65536(poly+(65536/32));
}

static void _ibc_1_131072( uint32_t *poly )
{
  _ibc_1_65536(poly);
  _ibc_1_65536(poly+(65536/32));
...
{
  uint32_t *poly_2048 = poly + (16384/32);
  // div by s[14] = [1, 4, 16, 64, 256, 1024, 4096, 16384]
  for(int i=0;i<(2048/32);i++) { poly[i+(4096/32)] ^= poly_2048[i]; }
  for(int i=0;i<(2048/32);i++) { poly[i+(1024/32)] ^= poly_2048[i]; }
  for(int i=0;i<(2048/32);i++) { poly[i+(256/32)] ^= poly_2048[i]; }
  for(int i=0;i<(2048/32);i++) { poly[i+(64/32)] ^= poly_2048[i]; }
  uint32_t todo = 0;
  for(int i=0;i<(2048/32);i++) {
    uint32_t qi = poly_2048[i];
    uint32_t pi = (qi<<1)^(qi<<4)^(qi<<16);
    uint32_t ri = (qi>>31)^(qi>>28)^(qi>>16);
    poly[i] ^= pi^todo;
    todo = ri;
  }
  poly[(2048/32)] ^= todo;

  bc_1(poly_2048, 2048/8);
  _bc_1_16384(poly);
}

void bc_1_32768_plus_4096(uint32_t *poly)
{
  uint32_t *poly_4096 = poly + (32768/32);
  // div by s[15] = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768]
  for(int i=0;i<(4096/32);i++) { poly[i+(16384/32)] ^= poly_4096[i]; }
  for(int i=0;i<(4096/32);i++) { poly[i+(8192/32)] ^= poly_4096[i]; }
  for(int i=0;i<(4096/32);i++) { poly[i+(4096/32)] ^= poly_4096[i]; }
  for(int i=0;i<(4096/32);i++) { poly[i+(2048/32)] ^= poly_4096[i]; }
  for(int i=0;i<(4096/32);i++) { poly[i+(1024/32)] ^= poly_4096[i]; }
  for(int i=0;i<(4096/32);i++) { poly[i+(512/32)] ^= poly_4096[i]; }
  for(int i=0;i<(4096/32);i++) { poly[i+(256/32)] ^= poly_4096[i]; }
  for(int i=0;i<(4096/32);i++) { poly[i+(128/32)] ^= poly_4096[i]; }
  for(int i=0;i<(4096/32);i++) { poly[i+(64/32)] ^= poly_4096[i]; }
  uint32_t todo = 0;
  for(int i=0;i<(4096/32);i++) {
    uint32_t qi = poly_4096[i];
    uint32_t pi = (qi<<1)^(qi<<2)^(qi<<4)^(qi<<8)^(qi<<16);
    uint32_t ri = (qi>>31)^(qi>>30)^(qi>>28)^(qi>>24)^(qi>>16)^qi;
    poly[i] ^= pi^todo;
    todo = ri;
  }
  poly[(4096/32)] ^= todo;

  bc_1(poly_4096, 4096/8);
  _bc_1_32768(poly);
}
