...
//static uint32_t radix_16_gf256_4x4(uint32_t a, uint32_t b) {}
#endif
...
void compute_elp_divstep(uint8_t * omega, uint8_t *sigma,  uint8_t *syndromes)
{
    ...
    for (int i = 0; i < 2*PARAM_DELTA; i++) {
        ...
        // for(int j = 0; j < 2*(PARAM_DELTA) +1; j++)
        // {
        //     g[j] = PQCLEAN_HQC256_CLEAN_gf_mul(g[j], f0) ^ PQCLEAN_HQC256_CLEAN_gf_mul(f[j], g0);
        // }
        ...
        // for(int j = 0; j < 2*(PARAM_DELTA) +1; j++)
        // {
        //     r[j] = PQCLEAN_HQC256_CLEAN_gf_mul(r[j], f0) ^ PQCLEAN_HQC256_CLEAN_gf_mul(v[j], g0);
        // }
...
void compute_error_values_new(uint8_t *error_values, const uint8_t *omega, const uint8_t *sigma, const uint8_t *error) {
    ...
    uint8_t beta_j_inverse[PARAM_DELTA+3] = {0};
    uint8_t e_j[PARAM_DELTA ] = {0};

    uint16_t delta_counter;
    uint16_t found;
    uint16_t mask1;
    uint16_t mask2;
    uint32_t tmpp1_array[(PARAM_DELTA + 3)>> 2] = {0};
    uint32_t tmpp2_array[(PARAM_DELTA + 3)>> 2] = {0};
    ...
    gf256_madd_32(tmpp1_array, beta_j_inverse_u32, omega[(PARAM_DELTA-1)]);
    gf256_madd_32(tmpp2_array, beta_j_inverse_u32, sigma_deriv[(PARAM_DELTA-1)]);
    tmpp1_array[0] ^= (uint32_t)omega[(PARAM_DELTA-2)] * 0x01010101u;
    tmpp2_array[0] ^= (uint32_t)sigma_deriv[(PARAM_DELTA-2)] * 0x01010101u;
    
    // pre-compute (uint32_t)omega[j] * 0x01010101u
    // pre-compute (uint32_t)sigma[j] * 0x01010101u
    uint32_t omegax01010101[PARAM_DELTA-2] = {0};
    uint32_t sigma_derivx01010101[PARAM_DELTA-2] = {0};
    uint32_t omegax01010101_rdx[PARAM_DELTA-2] = {0};
    uint32_t sigma_derivx01010101_rdx[PARAM_DELTA-2] = {0};
    ...
    for(int i = 0; i <= PARAM_DELTA-3; i += 2) {
        sigma_derivx01010101[i] = (uint32_t)sigma_deriv[i] * 0x01010101u;
        swap32(&sigma_derivx01010101_rdx[i], &sigma_derivx01010101[i]);
    }
    ...
    for(int i = 0; i < 8; i++)
    {
        ...
        for(int j = PARAM_DELTA-3; j >= 1; j-=2) // PARAM_DELTA = 29 in HQC-5
        {
            ...
            tmpp2_array[i] ^= sigma_derivx01010101_rdx[j];
            tmpp1_array[i] = radix_16_gf256_4x4(tmpp1_array[i], beta_j_inv_rdx);
            tmpp2_array[i] = radix_16_gf256_4x4(tmpp2_array[i], beta_j_inv_rdx);
            tmpp1_array[i] ^= omegax01010101_rdx[j-1];
            // skip below because they're all zeroes
            //tmpp2_array[i] ^= sigma_derivx01010101_rdx[j-1];
        }
        tmpp1_array[i] = radix_16_gf256_4x4(tmpp1_array[i], beta_j_inv_rdx);
        tmpp2_array[i] = radix_16_gf256_4x4(tmpp2_array[i], beta_j_inv_rdx);
        tmpp1_array[i] ^= omegax01010101_rdx[0];
        tmpp2_array[i] ^= sigma_derivx01010101_rdx[0];

        inv_swap32(&tmpp1_array[i]);
        inv_swap32(&tmpp2_array[i]);
    }
    
    uint8_t *tmpp1_array_bytes = (uint8_t *)tmpp1_array;
    uint8_t *tmpp2_array_bytes = (uint8_t *)tmpp2_array;
    for(int i = 0; i < PARAM_DELTA; i++)
    {
        e_j[i] = PQCLEAN_HQC256_CLEAN_gf_mul(tmpp1_array_bytes[i], PQCLEAN_HQC256_CLEAN_gf_inverse(tmpp2_array_bytes[i]));
    } 
