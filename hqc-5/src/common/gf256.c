...
/**
 * Implement GF256 multiply-accumulate on 60 elements packed into 15 registers.
...
void gf256_madd(uint32_t *c, const uint32_t *a, uint32_t b) {
    ...
    for(int j=0;j<15;j++) { regs[j] ^= bs_gf256_mul( a[j], m0 ); }
    for(int k=0;k<15;k++) { c[k] ^= regs[k]; }
}
 ...
 * c <- c + a*b
 */
void gf256_madd_32(uint32_t *c, const uint32_t *a, uint32_t b) {
    uint32_t regs[8] = {0};
    uint32_t m0[8];
    bs_gf256_multab(m0, (uint8_t)b);
...
/**
 * Implement GF256 multiplication on 60 elements packed into 15 registers.
