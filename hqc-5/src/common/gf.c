...
uint16_t PQCLEAN_HQC256_CLEAN_gf_mul(uint16_t a, uint16_t b) {
    ...
    uint32_t ab = ((uint32_t)a )| (((uint32_t)b) << 16);
    uint32_t rdx16_ab = rdx16_from_bitseq( ab );

    uint32_t rdx16_a = rdx16_ab & _NORM_RDX16_;
    uint32_t rdx16_b = (rdx16_ab >> 1) & _NORM_RDX16_;

    uint64_t rdx16_c = rdx16_gf256_mul_without_reduce( rdx16_a , rdx16_b );
    uint32_t c_lo = rdx16_c & 0xffffffff;
    uint32_t c_hi = rdx16_c >> 32;
    uint32_t rdx16_red = rdx16_gf256_reduce( c_hi&_NORM_RDX16_ );

    uint32_t res = rdx16_to_bitseq( (rdx16_red^c_lo) ) & 0xFF;
...
uint16_t PQCLEAN_HQC256_CLEAN_gf_square(uint16_t a) {
    union{
        uint64_t rdx16_a_sq;
        uint32_t rdx16_u32[2];
        uint8_t  byte[8];
    } uu;
    for(int i=0;i<8;i++) uu.byte[i]= (a>>i)&1;

    uint32_t res = rdx16_to_bitseq( rdx16_gf256_reduce( uu.rdx16_u32[1] )^uu.rdx16_u32[0] ) & 0xFF;
    return (uint16_t)res;
}
