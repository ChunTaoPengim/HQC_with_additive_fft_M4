...
{
	uint32_t pc_m32768[32768 / 32];
	uint32_t pc_m3072[3072 / 32];
	fafft_output((uint8_t*)pc_m32768, a_fft, b_fft);
	uint32_t temp2[96];
	gf2x_mul_3072_no_inv_swap(pc_m3072, (uint32_t*)a, (uint32_t*)b);

	poly_to_rd16(temp2, pc_m32768, 96);
	// the mod x^3072
	...
	// build pc_mix * IL
	gf2x_mul_3072_preswap(pc_m3072, IL_swapped_32768, pc_m3072);

	uint32_t pc_icrt0[(32768+3072) / 32] __attribute__((aligned(64))) = { 0 };

	// sparse polynomial
    {
        for (int i = 0; i < 96; i++) {
            for (int j = 0; j < minpoly_mod_lens[0]; j++) {
                pc_icrt0[i + minpoly_mods[0][j]] ^= pc_m3072[i];
            }
        }
    }
    ...
    for (int rem = 1; rem < 32; rem++) {
        for (int i = 0; i <= 96; i++) {
            uint32_t value = (i > 0) ? (pc_m3072[i - 1] >> (32 - rem)) : 0;
            if (i < 96) value |= pc_m3072[i] << rem;
	...
	{
		pc_icrt0[i] ^= pc_m32768[i];
	}

	memcpy(c, pc_icrt0, sizeof(uint32_t)*((32768+3072) / 32));
}
...
void crt_full(uint8_t * c, const uint8_t * a, const uint8_t * b)
{
	...
	fafft_input(a_u32, a);
	fafft_input(b_u32, b);
	fafft_output((uint8_t*)pc_m32768, a_u32, b_u32);
	uint32_t temp2[96];
	gf2x_mul_3072_no_inv_swap(pc_m3072, (uint32_t*)a, (uint32_t*)b);
