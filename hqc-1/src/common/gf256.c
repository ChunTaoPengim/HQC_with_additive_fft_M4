...
/**
 * Implement GF256 multiply-accumulate on 32 elements packed into 8 registers
...
void gf256_madd(uint32_t *c, const uint32_t *a, uint32_t b) {
    ...
    for(int j=0;j<8;j++) { regs[j] ^= bs_gf256_mul( a[j], m0 ); }
    for(int k=0;k<8;k++) { c[k] ^= regs[k]; }
}
...
/**
 * Implement GF256 multiplication on 32 elements packed into 8 registers
...
/**
 * Implement GF256 element-wise multiplication in radix-16 representation
 * Input: First multiplicand A (32-bit integer packing four elements (a3,a2,a1,a0) in radix-16 representation)
 * Input: Second multiplicand B (32-bit integer packing four elements (b3,b2,b1,b0) in radix-16 representation)
 * Output: C (32-bit integer packing results (c3,c2,c1,c0), where ci = ai × bi ∈ GF256)
 * 0x11d : 1_0001_1101 gf256
 */
